...
async function userFullfilled() {
    ...
    let year = new Intl.DateTimeFormat('en', { year: 'numeric' }).format(creationDatePreformatted)
    let month = new Intl.DateTimeFormat('en', { month: 'long' }).format(creationDatePreformatted)
    let day = new Intl.DateTimeFormat('en', { day: '2-digit' }).format(creationDatePreformatted)

    let hour = new Intl.DateTimeFormat('en', { hour: '2-digit', hour12: false }).format(creationDatePreformatted)
    let minute = new Intl.DateTimeFormat('en', { minute: '2-digit' }).format(creationDatePreformatted)
    let second = new Intl.DateTimeFormat('en', { second: '2-digit' }).format(creationDatePreformatted)
...
document.getElementById("code-nav-button").addEventListener("mouseover", () => {
    document.getElementById("tooltip-text-container").style.opacity = 1;
    document.getElementById("tooltip-text").textContent = "Look through my spaghetti code";
})
...
document.getElementById("users-nav-button").addEventListener("click", function mainPageAnimation() {
